...
function signed(event) { const payload = JSON.stringify(event, null, 2); return { payload, signature: crypto.createHmac('sha256', secret).update(payload).digest('hex') }; }
...
(async () => {
  try {
    ...
    const overRefund = await request(`/api/payments/${intent.id}/refunds`, { method: 'POST', headers: auth, body: JSON.stringify({ amount: '1.00' }) });
    assert.equal(overRefund.response.status, 409);
