...
(() => {
  ...
  const state = {
    ...
    kycMessage: '',
    riskStatus: null,
    admin: { kyc: [], cases: [], accounts: [] },
    adminBusy: false,
    adminError: '',
    adminMessage: '',
    bootstrapping: Boolean(localStorage.getItem(TOKEN_KEY)),
  };
  ...
  async function bootstrap() {
    if (!state.token) { state.bootstrapping = false; return render(); }
    render();
    try {
  ...
  async function loadIdentityAndRisk() {
    try { state.kyc = (await api('/api/identity')).identity || null; } catch { state.kyc = null; }
    try { state.riskStatus = await api('/api/risk/status'); } catch { state.riskStatus = null; }
  }

  async function loadAdminData() {
    if (!state.user?.isAdmin) return;
    try {
      const [kyc, cases] = await Promise.all([api('/api/admin/identity/review'), api('/api/admin/risk/cases')]);
      state.admin.kyc = kyc.checks || [];
      state.admin.cases = cases.cases || [];
      state.adminError = '';
  ...
  function statusLabel(status) {
    return { verified: 'Verified', rejected: 'Rejected', needs_review: 'Needs review', pending: 'Pending', expired: 'Expired', allow: 'No issues found', review: 'Under review', deny: 'Blocked' }[status] || 'Not started';
  }

  function renderIdentityPanel() {
    const section = el('div', 'section');
    const title = el('div', 'section__title'); title.textContent = 'Identity check'; section.appendChild(title);
    const panel = el('div', 'send-panel identity-panel');
    const status = state.kyc?.status || 'not_started';
    const badge = el('div', `status-badge status-badge--${status}`); badge.textContent = statusLabel(status); panel.appendChild(badge);
    const copy = el('p', 'panel-copy');
    copy.textContent = status === 'verified' ? 'Your demo identity check is verified.' : status === 'rejected' ? 'Your identity check was rejected. You can submit a new demo check.' : status === 'needs_review' ? 'Your check needs a manual review. No action is required right now.' : status === 'pending' ? 'Your demo identity check is being processed.' : status === 'expired' ? 'Your previous verification has expired.' : 'Complete a demo identity check to unlock the verification experience.';
    panel.appendChild(copy);
    if (!['verified','pending','needs_review'].includes(status)) {
      const form = el('form');
      const note = field('kycNote', 'Optional note', 'text', { placeholder: 'Use the demo flow' }); note.querySelector('input').required = false;
      const outcome = el('div', 'field'); const outcomeLabel = el('label'); outcomeLabel.textContent = 'Demo outcome'; outcomeLabel.setAttribute('for', 'kyc-outcome'); const select = el('select'); select.id = 'kyc-outcome'; select.name = 'kycOutcome'; [['verified','Verified'],['pending','Pending'],['rejected','Rejected'],['needs_review','Needs review']].forEach(([value, text]) => { const option = el('option'); option.value = value; option.textContent = text; select.appendChild(option); }); outcome.append(outcomeLabel, select);
      const submit = el('button', 'btn'); submit.type = 'submit'; submit.disabled = state.kycBusy; submit.innerHTML = state.kycBusy ? '<span class="spinner"></span> Starting…' : 'Start demo check →';
      form.append(note, outcome, submit); form.addEventListener('submit', async (event) => {
        event.preventDefault(); state.kycBusy = true; state.kycError = ''; state.kycMessage = ''; render();
        try { const result = await api('/api/identity/kyc', { method: 'POST', headers: { 'Idempotency-Key': cryptoKey() }, body: JSON.stringify({ note: new FormData(event.target).get('kycNote') || '', demoOutcome: new FormData(event.target).get('kycOutcome') }) }); state.kyc = result.check; state.kycMessage = 'Demo identity check submitted.'; }
        catch (error) { state.kycError = error.message; } finally { state.kycBusy = false; render(); }
      }); panel.appendChild(form);
    }
    if (state.kycError) { const err = el('div', 'form-error'); err.setAttribute('role', 'alert'); err.textContent = state.kycError; panel.appendChild(err); }
    if (state.kycMessage) { const ok = el('div', 'form-success'); ok.textContent = state.kycMessage; panel.appendChild(ok); }
    section.appendChild(panel); return section;
  }

  function renderRiskPanel() {
    const section = el('div', 'section'); const title = el('div', 'section__title'); title.textContent = 'Account safety'; section.appendChild(title);
    const panel = el('div', 'send-panel'); const status = state.riskStatus?.status || 'allow';
    const badge = el('div', `status-badge status-badge--${status}`); badge.textContent = statusLabel(status); panel.appendChild(badge);
    const copy = el('p', 'panel-copy'); copy.textContent = status === 'review' ? 'One of your recent actions is under review. We will update this status when the review is complete.' : status === 'deny' ? 'A recent action was blocked to help protect your account. Please contact support if you think this is a mistake.' : 'Your account has no active safety review.'; panel.appendChild(copy); section.appendChild(panel); return section;
  }

  function renderAdminPanel() {
    if (!state.user?.isAdmin) return null;
    const section = el('div', 'section admin-panel'); const title = el('div', 'section__title'); title.textContent = 'Admin review desk'; section.appendChild(title);
    const panel = el('div', 'send-panel');
    if (state.adminError) { const err = el('div', 'form-error'); err.textContent = state.adminError; panel.appendChild(err); }
    const heading = el('h3'); heading.textContent = 'KYC queue'; panel.appendChild(heading);
    if (!state.admin.kyc.length) { const empty = el('p', 'panel-copy'); empty.textContent = 'No identity checks need review.'; panel.appendChild(empty); }
    state.admin.kyc.forEach(check => panel.appendChild(renderAdminKyc(check)));
    const riskHeading = el('h3'); riskHeading.textContent = 'Risk cases'; panel.appendChild(riskHeading);
    if (!state.admin.cases.length) { const empty = el('p', 'panel-copy'); empty.textContent = 'No open risk cases.'; panel.appendChild(empty); }
    state.admin.cases.forEach(item => panel.appendChild(renderAdminCase(item)));
    section.appendChild(panel); return section;
  }

  function adminButton(label, onClick, tone = '') { const button = el('button', `btn btn--small ${tone}`); button.type = 'button'; button.textContent = label; button.disabled = state.adminBusy; button.addEventListener('click', onClick); return button; }
  async function adminAction(path, body, message) { state.adminBusy = true; state.adminError = ''; state.adminMessage = ''; render(); try { await api(path, { method: 'POST', body: JSON.stringify(body) }); state.adminMessage = message; await loadAdminData(); } catch (error) { state.adminError = error.message; } finally { state.adminBusy = false; render(); } }
  function renderAdminKyc(check) { const row = el('div', 'admin-row'); const text = el('div', 'admin-row__text'); text.textContent = `${check.email || check.username} · ${statusLabel(check.status)}`; const actions = el('div', 'admin-row__actions'); actions.append(adminButton('Allow', () => adminAction(`/api/admin/identity/${encodeURIComponent(check.id)}/decision`, { decision: 'allow' }, 'Identity check approved.'), ''), adminButton('Needs review', () => adminAction(`/api/admin/identity/${encodeURIComponent(check.id)}/decision`, { decision: 'review' }, 'Identity check kept under review.'), 'btn--ghost'), adminButton('Deny', () => { if (window.confirm('Deny this identity check?')) adminAction(`/api/admin/identity/${encodeURIComponent(check.id)}/decision`, { decision: 'deny' }, 'Identity check denied.'); }, 'btn--danger')); row.append(text, actions); return row; }
  function renderAdminCase(item) { const row = el('div', 'admin-row'); const text = el('div', 'admin-row__text'); text.textContent = `${item.email || item.username} · ${item.severity} · ${statusLabel(item.status)}`; const actions = el('div', 'admin-row__actions'); actions.append(adminButton('Allow', () => adminAction(`/api/admin/risk/cases/${encodeURIComponent(item.id)}/decision`, { decision: 'allow' }, 'Risk case allowed.'), ''), adminButton('Deny', () => { if (window.confirm('Deny and resolve this risk case?')) adminAction(`/api/admin/risk/cases/${encodeURIComponent(item.id)}/decision`, { decision: 'deny' }, 'Risk case denied.'); }, 'btn--danger'), adminButton('Dismiss', () => { if (window.confirm('Dismiss this risk case?')) adminAction(`/api/admin/risk/cases/${encodeURIComponent(item.id)}/dismiss`, { resolution: 'dismissed by admin' }, 'Risk case dismissed.'); }, 'btn--ghost')); row.append(text, actions); return row; }
  ...
  function renderDashboard() {
    ...
    $app.appendChild(renderRiskPanel());
    const adminPanel = renderAdminPanel(); if (adminPanel) $app.appendChild(adminPanel);
  ...
  function renderPaymentPanel() {
    ...
    const outcomeLabel = el('label'); outcomeLabel.textContent = 'Demo outcome'; outcomeLabel.setAttribute('for', 'demo-outcome');
    const select = el('select'); select.id = 'demo-outcome'; select.name = 'demoOutcome';
  ...
  function render() {
    if (state.bootstrapping) {
      $app.innerHTML = '<div class="loading-shell" role="status" aria-live="polite"><span class="spinner spinner--dark"></span><p>Loading your Nota wallet…</p></div>';
      return;
    }
