...
function migrate(db) {
  ...
  // Phase 9 sandbox payment and transfer records.
  db.exec(`
    CREATE TABLE IF NOT EXISTS sandbox_transfers (
      ...
      amount_cents INTEGER NOT NULL CHECK(amount_cents > 0),
      currency TEXT NOT NULL DEFAULT 'EUR',
      status TEXT NOT NULL CHECK(status IN ('pending','succeeded','failed','cancelled','under_review')),
      idempotency_key TEXT NOT NULL,
    ...
    CREATE INDEX IF NOT EXISTS sandbox_transfers_user_idx ON sandbox_transfers(user_id, created_at);
    CREATE INDEX IF NOT EXISTS sandbox_transfers_recipient_idx ON sandbox_transfers(recipient_id, created_at);
