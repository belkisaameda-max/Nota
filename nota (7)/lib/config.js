...
function loadConfig(env = process.env) {
  ...
  const paymentMode = String(env.PAYMENT_MODE || 'sandbox').toLowerCase();
  if (!['sandbox', 'demo'].includes(paymentMode)) problems.push('PAYMENT_MODE must be sandbox or demo; real payment processing is not supported.');
  if (problems.length) throw new Error(`Refusing to start due to configuration problems:\n - ${problems.join('\n - ')}`);
  ...
  return {
    ...
    adminUsernames: new Set(String(env.ADMIN_USERNAMES || 'amer').split(',').map((value) => value.trim().toLowerCase()).filter(Boolean)),
  };
}
