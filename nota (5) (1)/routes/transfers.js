...
function registerTransferRoutes(app, { db, auth, audit, idempotency, risk }) {
  function ensureActive(userId) {
    const account = db.prepare('SELECT account_status FROM users WHERE id=?').get(Number(userId));
    if (account?.account_status && account.account_status !== 'active') throw ApiError.forbidden('This account cannot move test credits.', 'ACCOUNT_RESTRICTED');
  }
  ...
  app.post('/api/transfers', auth, asyncHandler(async (req, res) => {
    ...
    const riskResult = risk.evaluate({ userId: req.user.sub, evaluationType: 'transfer', subjectType: 'transfer', subjectId: key, idempotencyKey: `transfer:${req.user.sub}:${key}`, scenario: req.body.demoRiskScenario || 'LOW_RISK', signals: { amountCents: amount } });
    if (riskResult.evaluation.decision === 'deny') throw ApiError.forbidden('This transfer was declined by Nota risk controls.', 'RISK_DENIED');
    if (riskResult.evaluation.decision === 'review') throw ApiError.conflict('This transfer is under review and was not sent.', 'RISK_REVIEW');
